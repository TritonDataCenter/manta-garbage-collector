...
function getMorayConfig(opts) {
    ...
    var labeledCollector;
    var morayConfig;

    if (opts.collector) {
        //
        // This is pretty gross, but we want to be able to distinguish between
        // moray and electric-moray's fast_* metrics and we have no control over
        // those since they happen in node-fast, which gets passed the
        // 'collector' we use here by the moray client. As such we pass a
        // "collector" which just makes a small change to each created metric
        // and adds a label when creating a counter, gauge or histogram.
        //
        // eslint-disable-next-line no-inner-declarations
        function _createCollectorWithLabel(func, _opts) {
            // Copy the options so we don't have side-effects for the caller.
            var optsCopy = jsprim.deepCopy(_opts);

            if (!optsCopy.labels) {
                optsCopy.labels = {};
            }
            optsCopy.labels.clientType = opts.clientType || 'moray';

            return opts.collector[func](optsCopy);
        }

        labeledCollector = {
            FIXED_BUCKETS: opts.collector.FIXED_BUCKETS,
            counter: function _counter(_opts) {
                return _createCollectorWithLabel('counter', _opts);
            },
            gauge: function _gauge(_opts) {
                return _createCollectorWithLabel('gauge', _opts);
            },
            histogram: function _histogram(_opts) {
                return _createCollectorWithLabel('histogram', _opts);
            }
        };
    }
