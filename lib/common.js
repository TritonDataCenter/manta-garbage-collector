...
function loadConfig(opts, callback) {
    ...
    fs.readFile(opts.filename, function _onReadFile(err, data) {
        ...
        // MANTA-5424 we need to be able to use storinfo to check that storage
        // ids are still valid, but we can't rely on the storinfo_url since that
        // might not have been set in SAPI. So if it's not set, we generate the
        // storinfo_url from our service_name by replacing `garbage-collector`
        // with `storinfo`.
        if (!parsed.storinfo_url || parsed.storinfo_url.length === 0) {
            assert.string(parsed.service_name, 'config must have service_name');
            parsed.storinfo_url = parsed.service_name.replace(/^garbage-collector/, 'storinfo');
        }
