...
function getEMorayConfig(opts) {
    ...
    if (opts.collector) {
        ...
        // and adds an 'e' prefix to fast_client_* keys when creating a counter,
        // gauge or histogram.
        //
        // eslint-disable-next-line no-inner-declarations
        function _createCollectorWithLabel(func, _opts) {
            // Copy the options so we don't have side-effects for the caller.
            var _optsCopy = jsprim.deepCopy(_opts);

            if (
                _optsCopy.name &&
                _optsCopy.name.substr(0, 12) === 'fast_client_'
            ) {
                _optsCopy.name = 'e' + _optsCopy.name;
            }

            return opts.collector[func](_optsCopy);
        }
