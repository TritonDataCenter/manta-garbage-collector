...
function GarbageMpuCleaner(opts) {
    ...
    //
    self.mpuCleanupAgeSecs = self.config.options.mpu_cleanup_age_seconds || 300; // XXX default 1h?
            ...
            funcs: [
                ...
                function _deleteMetadata(ctx, cb) {
                    ...
                    if (ctx.records.length === 0) {
                        // XXX debug?
                        self.log.info(
                            'No records received, nothing to delete.'
                        );
                ...
                function _deleteMantaUploadRecords(ctx, cb) {
                    ...
                    if (ctx.successfullyDeleted.length === 0) {
                        self.log.info(
                            ...
                            'Did not delete any manta records, not removing manta_upload record'
                        );
                        cb();
                        return;
                    }
                    ...
                    vasync.forEachPipeline(
                        {
                            func: function(rec, _cb) {
                                self.log.info(
                                    {rec: rec},
                                    'Would delete manta_upload record'
                                );
                        ...
                        function _deletedMantaUploadRecords(err) {
                            ...
                            self.log.info(
                                ...
                                'Did _deleteMantaUploadRecords'
                            );

                            self.addCounter(
                                'morayFinalizeDeleteCount',
                                ctx.successfullyDeleted.length
                            );
                            self.addCounter(
