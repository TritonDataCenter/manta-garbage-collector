...
// Future:
//
//  - If we read a full batch (record_read_batch_size results) we probably
//    should decrease the delay since there are likely to be more records
//    waiting.
...
function GarbageDirConsumer(opts) {
    ...
    //
    // record_read_wait_interval sets the number of *milliseconds* we wait after each
    // run completes before we start the next one. Each run will do a
    ...
    //
    self.readBatchDelay = self.config.options.record_read_wait_interval || 60000;
    self.readBatchSize = self.config.options.record_read_batch_size || 200;
...
GarbageDirConsumer.prototype.run = function run(callback) {
    ...
    vasync.pipeline(
        ...
        function _onPipeline(err) {
            ...
            // Schedule next run.
            self.log.info('Will run again in %d seconds.',
