...
// We'll update the "fastdelete_records_outstanding_count" metrics at most
// every MAX_COUNT_UPDATE_FREQ_MS milliseconds.
var MAX_COUNT_UPDATE_FREQ_MS = 60000;
...
function GarbageDirConsumer(opts) {
    ...
    if (self.metricsManager) {
        self.metrics = {
            fastdeleteRecordsOutstanding: self.metricsManager.collector.gauge({
                name: metricPrefix + 'fastdelete_records_outstanding_count',
                help:
                    'Number of records last seen remaining in the ' +
    ...
    vasync.pipeline(
        {
            arg: {},
            funcs: [
                function _readCount(_, cb) {
                    var now = Date.now();

                    if (
                        now - self.lastGarbageCount >
                        MAX_COUNT_UPDATE_FREQ_MS
                    ) {
                        self.lastGarbageCount = now;
                        self.readGarbageCount(cb);
                    } else {
...
GarbageDirConsumer.prototype.readGarbageCount = function readGarbageCount(
    callback
) {
    var self = this;
    ...
    //
    // This is the fastest way we've been able to find to get a count and while
    // it's "approximate" it's good enough for this metric. Doing a count(*) on
    // a large table is really slow.
    //
    // See also: https://wiki.postgresql.org/wiki/Count_estimate
    //
    req = self.client.sql(
        "SELECT reltuples as approximate_row_count FROM pg_class WHERE relname = 'manta_fastdelete_queue'"
    );
    ...
    req.once('error', function _findError(err) {
        self.log.error(
            ...
            'Error reading garbage count.'
        );

        // Nothing we can really do about this error, we'll just hope it works
        // next time and *not* return an error here.
        callback();
    });

    req.on('record', function _findRecord(obj) {
        try {
            assert.number(obj.approximate_row_count, 'approximate_row_count');
            self.setGauge(
                'fastdeleteRecordsOutstanding',
                obj.approximate_row_count,
                {
                    shard: self.shard
                }
            );
            self.log.trace(
                {
                    count: obj.approximate_row_count,
                    elapsed: elapsedSince(beginning),
                    shard: self.shard
                },
                'Got approximate_row_count for shard'
            );
        } catch (e) {
            self.log.warn({err: e}, 'Failed to read garbage count.');
        }
    });
...
GarbageDirConsumer.prototype.readGarbageBatch = function readGarbageBatch(
    ...
    // could end up reading the same records again. This means if a bad record
    // is breaking us, we might not make progress. But we'll have logged errors and
    // error metrics will be counting up so hopefully these cases can be tracked
    // down and fixed before they become problems. Since we've changed to not
    // doing sorted results however, this seems less likely.
    //
    ...
    req = self.client.sql(
        'SELECT * FROM manta_fastdelete_queue LIMIT ' + self.readBatchSize
    );
    ...
    req.on('record', function _findRecord(obj) {
        ...
        var idx;
        var storageId;
        var value = JSON.parse(obj._value); // Aborts if JSON.parse fails
