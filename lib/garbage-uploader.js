...
var FAILED_SLOW_PUT_CUTOFF = 5; // seconds after which a failed PUT will be considered "slow"
var FAILED_SLOW_PUT_FAILURES = 10; // number of consecutive PUT failures before ignoring a storage Id
// var FAILED_SLOW_PUT_DISABLE_TIME = 24 * 60 * 60 * 1000; // ms to disable a server after failures (24h)
var FAILED_SLOW_PUT_DISABLE_TIME = 2 * 60 * 1000; // ms to disable a server after failures (24h)
...
GarbageUploader.prototype.run = function run(callback) {
    ...
    self.log.info({dirty: self.dirtyStorageZones}, 'Running GarbageUploader.');
...
GarbageUploader.prototype.enableStorageId = function enableStorageId(storageId) {
    var self = this;
    ...
    if (disableTimestamp) {
        ...
        self.log.info('ENABLED ' + storageId + ' which was disabled at ' + disableTimestamp);
    }
...
GarbageUploader.prototype.disableStorageId = function disableStorageId(storageId, disableTime) {
    var self = this;
    ...
    // right away and only do so after we've got failures again.
    ...
    if (disableTime > 0) {
        ...
        self.log.info('DISABLED ' + storageId + ' will reenable after at ' + disableTime);
    }
};
...
GarbageUploader.prototype.putInstructionFile = function putInstructionFile(
    ...
    if (self.disabledStorageIds[storageId]) {
        self.log.info('IGNORE PUT FOR DISABLED STORAGEID: ' + storageId);
        callback();
        return;
    }
    ...
    function _handlePutFail(err, req, res) {
        ...
        if (elapsed > FAILED_SLOW_PUT_CUTOFF) {
            self.storageServerFailedSlowPuts[storageId]++;

            self.log.info('SLOW PUT: ' + elapsed + ': ' + self.storageServerFailedSlowPuts[storageId]);

            if (self.storageServerFailedSlowPuts[storageId] >= FAILED_SLOW_PUT_FAILURES) {
                self.log.warn('Too many consecutive PUT failures, disabling uploads for ' + storageId);
                self.disableStorageId(storageId, FAILED_SLOW_PUT_DISABLE_TIME);
            }
        }
    }
    ...
    client.put(remotePath, function _onPut(err, req) {
        ...
        if (err) {
            self.log.error('FAILED RIGHT AFTER PUT');
        ...
        req.once('result', function _onResult(e, res) {
            ...
            if (e) {
                self.log.error('FAILED IN REQ.ONCE');
                ...
                self.log.error('FAILED != 201');
                newErr = new Error('PUT Failed: ' + res.statusCode + ': ' + res.statusMessage);
