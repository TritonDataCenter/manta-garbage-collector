...
function GarbageUploader(opts) {
    ...
    self.slowPutMaxFailures =
        self.config.options.dir_slow_put_max_failures || DEFAULT_SLOW_PUT_MAX_FAILURES;
    self.slowPutDisableTime =
        self.config.options.dir_slow_put_disable_time || DEFAULT_SLOW_PUT_DISABLE_TIME;
...
GarbageUploader.prototype.disableStorageId = function disableStorageId(
    ...
    if (disableTime > 0) {
        ...
        self.log.debug(
            'Disabled ' + storageId + ' will re-enable in ' + disableTime + ' ms'
        );
    }
};
